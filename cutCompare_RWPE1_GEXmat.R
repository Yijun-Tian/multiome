...
cutmat <- fread("RWPE1e.cut.mat.txt", header = T)
...
for (i in 2:dim(cutmat)[1]) {
  ...
  try(s <- subset(RWPE1, features = genelist), silent = T)
...
saveRDS(results, "RWPE1e.results.rds")
write.table(results, row.names = F,"RWPE1e.results.txt", sep = "\t", quote = F)
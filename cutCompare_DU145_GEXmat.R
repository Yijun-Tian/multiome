...
cutmat <- fread("DU145e.cut.mat.txt", header = T)
...
for (i in 2:dim(cutmat)[1]) {
  ...
  try(s <- subset(DU145, features = genelist), silent = T)
...
saveRDS(results, "DU145e.results.rds")
write.table(results, row.names = F,"DU145e.results.txt", sep = "\t", quote = F)
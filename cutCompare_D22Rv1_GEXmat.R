...
cutmat <- fread("D22Rv1e.cut.mat.txt", header = T)
...
for (i in 2:dim(cutmat)[1]) {
  ...
  try(s <- subset(D22Rv1, features = genelist), silent = T)
...
saveRDS(results, "D22Rv1e.results.rds")
write.table(results, row.names = F,"D22Rv1e.results.txt", sep = "\t", quote = F)
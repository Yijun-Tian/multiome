...
cutmat <- fread("MLNCaPe.cut.mat.txt", header = T)
...
for (i in 2:dim(cutmat)[1]) {
  ...
  try(s <- subset(MLNCaP, features = genelist), silent = T)
...
saveRDS(results, "MLNCaPe.results.rds")
write.table(results, row.names = F,"MLNCaPe.results.txt", sep = "\t", quote = F)
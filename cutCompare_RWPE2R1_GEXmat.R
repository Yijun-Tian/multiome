...
cutmat <- fread("RWPE2R1e.cut.mat.txt", header = T)
...
for (i in 2:dim(cutmat)[1]) {
  ...
  try(s <- subset(RWPE2R1, features = genelist), silent = T)
...
saveRDS(results, "RWPE2R1e.results.rds")
write.table(results, row.names = F,"RWPE2R1e.results.txt", sep = "\t", quote = F)
...
cutmat <- fread("RWPE1R2e.cut.mat.txt", header = T)
...
for (i in 2:dim(cutmat)[1]) {
  ...
  try(s <- subset(RWPE1R2, features = genelist), silent = T)
...
saveRDS(results, "RWPE1R2e.results.rds")
write.table(results, row.names = F,"RWPE1R2e.results.txt", sep = "\t", quote = F)
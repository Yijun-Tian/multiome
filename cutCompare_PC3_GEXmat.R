...
cutmat <- fread("PC3e.cut.mat.txt", header = T)
...
for (i in 2:dim(cutmat)[1]) {
  ...
  try(s <- subset(PC3, features = genelist), silent = T)
...
saveRDS(results, "PC3e.results.rds")
write.table(results, row.names = F,"PC3e.results.txt", sep = "\t", quote = F)
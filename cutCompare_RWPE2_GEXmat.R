...
cutmat <- fread("RWPE2e.cut.mat.txt", header = T)
...
for (i in 2:dim(cutmat)[1]) {
  ...
  try(s <- subset(RWPE2, features = genelist), silent = T)
...
saveRDS(results, "RWPE2e.results.rds")
write.table(results, row.names = F,"RWPE2e.results.txt", sep = "\t", quote = F)
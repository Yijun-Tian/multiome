...
cutmat <- fread("M22Rv1e.cut.mat.txt", header = T)
...
for (i in 2:dim(cutmat)[1]) {
  ...
  try(s <- subset(M22Rv1, features = genelist), silent = T)
...
saveRDS(results, "M22Rv1e.results.rds")
write.table(results, row.names = F,"M22Rv1e.results.txt", sep = "\t", quote = F)
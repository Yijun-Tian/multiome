...
cutmat <- fread("PrECe.cut.mat.txt", header = T)
...
for (i in 2:dim(cutmat)[1]) {
  ...
  try(s <- subset(PrEC, features = genelist), silent = T)
...
saveRDS(results, "PrECe.results.rds")
write.table(results, row.names = F,"PrECe.results.txt", sep = "\t", quote = F)
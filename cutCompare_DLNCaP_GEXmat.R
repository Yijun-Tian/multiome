...
cutmat <- fread("DLNCaPe.cut.mat.txt", header = T)
...
for (i in 2:dim(cutmat)[1]) {
  ...
  try(s <- subset(DLNCaP, features = genelist), silent = T)
...
saveRDS(results, "DLNCaPe.results.rds")
write.table(results, row.names = F,"DLNCaPe.results.txt", sep = "\t", quote = F)